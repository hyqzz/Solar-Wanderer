...
# R9 需求/问题分析（2026-06-12）

## 摘要
两大类：① 镜头/操控系统五项缺陷与一项新视角需求；② 登陆渲染闪烁 + 地表/大气/海洋逼真度 + 全天体真实形态排查。

## 根因分析（已通过代码审读定位）

### R9-1a 行走仰望太灵敏 + 概率性快速旋转
- `Input.mousemove` 每**事件**钳制 ±150px，但一帧可含多个事件，**每帧累计无上限**；
- 卡顿帧（地形重建等）dt 大 → 平滑系数 `1-exp(-dt/τ)→1`，积累输入一帧内全量灌入 → 快速旋转；
- 自动登陆时 `requestPointerLock()` 获得锁定的瞬间 Chromium 产生突发 movement 事件串；
- 行走灵敏度 0.0011 恒定，对"仰望定位"仍偏高。

### R9-1b 平移后滚轮缩放概率性跳回地球（核心缺陷）
`orbitCamera.update` 离轴推拉每帧调用 `env.centerDepth` **实时重定目标**：
屏幕中心射线命中哪个天体就以它为收敛深度。panOffset 存储在**焦点体固系**（随地球自转 15°/h 旋转）→ 视线缓慢漂移 → 射线概率性扫过地球盘面 → 参考深度从"太阳 1 AU"瞬间切为"地球近距"，而累计比例 `_dolly` 不变 → `T=depth×_dolly` 突变，单帧产生指向地球的巨大位移 = "瞬间回到地球"。命中↔丢失交替还会来回猛冲。

### R9-1c 远离无上限 + 灵敏度无由低到高
缩放上限 7.5e10 km（≈500 AU）远超日球层顶（121 AU）；滚轮每格固定 ×1.25、按键固定速率，无"轻拨精细、连滚加速"的低→高渐进。

### R9-1d 平移+滚轮无法达成登陆
所有登陆语义（minDist 地形下限、自动倾斜、自动转行走、拖拽灵敏度）全部绑定 `focusId`。平移+滚轮接近另一天体时**焦点从不交接** → 永远无法触发该天体的着陆链路；这也是 1b 状态机脆弱的架构根源。

### R9-2a 登陆渲染闪烁（三重根因）
1. **Float32 顶点量化（主因）**：地形顶点为天体本地坐标，模 ≈ R（地球 6371 km）。fp32 在 6.4e6 m 处精度仅 ~0.5 m，眼高 1.7 m 时顶点/矩阵运算量化误差与视点同量级 → 闪烁、起伏跳变；
2. LOD 环带重叠区（内环外延 0.92 重叠）两层近共面网格 z-fight；
3. 海面 = 基底球面 +1 m，与基底球 z-fight。

### R9-2b 大气/海洋
穿云无表现；海洋只是蓝色硬地板，无法入水，无水下环境。

### R9-2c 天体真实性
- **pluto.jpg 实为 HTML 错误页**（下载存错），加载失败回退程序化纹理 → 无心形（斯普特尼克平原）；
- 火卫一/火卫二渲染为正球体，真实为 27×22×18 km / 15×12×11 km 土豆状不规则体；引擎无不规则形状支持；
- titan.jpg / callisto.jpg manifest=false（程序化兜底，前轮已接受，本轮一并重试下载）。

## 最小复现条件
| 缺陷 | 复现 |
|------|------|
| 1a | 单帧注入 1200px 累计输入 + dt=0.12 卡顿帧 → 单帧偏航跳变超阈值 |
| 1b | 离轴推拉中 centerDepth 在两天体间交替返回 → 单帧位移突变 |
| 1c | 持续 PageDown → dist 突破 121 AU 全景距离 |
| 1d | 平移把太阳居中+滚轮收敛 → focusId 仍为 earth |
| 2a | Math.fround(6371.0017) − 6371.0017 ≥ 0.25 m 数值演示 |
| 2c | pluto.jpg 魔数 ≠ FFD8FF；HeightField 无法表达三轴椭球 |

## 修复范围边界
- **改**：orbitCamera.js（缩放/焦点交接/上限/惯性系视角）、ship.js（行走视角钳制+锁定静默）、main.js（centerDepth 带 id、焦点交接回调、水下/穿云环境）、terrain.js（锚点相对坐标+浮动原点注册+海洋/水下）、builder.js（不规则形状）、bodies.js（dims 数据）、fetch-textures.mjs（pluto 源）
- **不碰**：星历层 astro/（除 bodies.js 数据字段）、atmosphere.js 散射核心、quality.js、UI 层结构

## 边界情况清单
1. 焦点交接瞬间视向/位置必须严格连续（adoptPosition+quat）；
2. 交接后若新焦点不可登陆（太阳/气巨）→ minDist 语义随之切换，不得穿入；
3. 行走钳制不得破坏正常快速甩头（每帧上限应远大于正常移动）;
4. 不规则体行走碰撞 = 视觉网格（同一 HeightField）；
5. 地形锚点重定位（>2km）瞬间不得产生可见跳变；
6. 水下模式在无海洋天体（月球）绝不触发；
7. 惯性系视角下极点穿越/拖拽/缩放语义保持一致。

```mermaid
flowchart LR
  A[平移+滚轮接近新天体] --> B{中心命中天体 B?}
  B -->|是,锁定该目标| C[沿视线收敛到 B]
  C --> D{接近度达阈值?}
  D -->|是| E[焦点交接 adoptPosition→B 视向连续]
  E --> F[径向缩放/自动倾斜/自动登陆链路全部生效]
  F --> G[一路滚轮直达地表行走]
```
