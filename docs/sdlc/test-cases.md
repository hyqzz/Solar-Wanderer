...
| 用例ID | 类型 | 描述 | 预期 | 优先级 |
|--------|------|------|------|--------|
| R9-U01 | 复现 | 卡顿帧+1200px 堆积输入 | 单帧俯仰 ≤0.12 rad | P0 |
| R9-U02 | 回归 | 正常 30px/帧环视 1s | 俯仰 >0.5 rad（钳制不影响正常操作） | P0 |
| R9-U03 | 复现 | 平移居中太阳+滚轮 240 帧 | 单帧推进 ≤剩余 18%、零倒退、显著接近 | P0 |
| R9-U04 | 复现 | 持续缩出 2000 帧 | ≤260 AU | P1 |
| R9-U05 | 复现 | 太阳居中收敛 | focusId 交接为 sun | P0 |
| R9-U06 | 复现 | terrain.js 静态检查 | uPatchRel+origin 相对几何存在 | P0 |
| R9-U07 | 复现 | HeightField shape.dims | 火卫一长短轴比 >1.2 | P1 |
| R9-U08 | 复现 | pluto.jpg 魔数 | FFD8FF | P1 |
| R9-E01 | 端到端 | 地球海洋下潜 | 浸没层 >0.2 → 海床自动行走 | P0 |
| R9-E02 | 端到端 | V 惯性模式切换 | 位置 <50km / 视向 <2° 连续；加速时间木卫一绕转 | P1 |
| R9-E03 | 回归 | R7 探针（登陆/起飞/入气） | 全过 | P0 |
| R9-E04 | 回归 | R8 缩放三场景 + R7 六断言 + 单元 33 | 全过 | P0 |
