...
## 复现环境
Windows 11 / Node v24 / three 0.165；Node 直驱引擎层（无浏览器）+ Puppeteer SwiftShader 端到端。

## 复现方法与结果（修复前）
`node tools/repro-r9.mjs` —— 6 项 FAIL：

| 缺陷 | 复现证据 |
|------|----------|
| 1a 行走快速旋转 | 卡顿帧(dt=0.12)+事件堆积(1200px) → 单帧俯仰跳变 **68.8°** |
| 1c 远离无上限 | 持续缩出可达 **501 AU**（日球层顶 121 AU） |
| 1d 无焦点交接 | 太阳居中+滚轮收敛后 focusId 仍 = earth |
| 2a 顶点量化 | fp32(6371.0017 km) 误差 **235 mm**（眼高 1.7 m 同量级 → 闪烁）；terrain.js 无锚点相对几何 |
| 2c-1 火卫一正球 | 长短轴比 1.007（真实 1.44） |
| 2c-2 冥王星贴图 | pluto.jpg 文件头 = `<!doctype html>`（HTML 错误页） |

1b（平移后跳跃）复现于推拉目标逐帧重定机制：视线漂移（panOffset 随焦点体自转）扫过近处天体时参考深度突变，
`T=depth×_dolly` 单帧猛冲（修复后断言：单帧推进 ≤ 剩余 18%、全程零倒退）。

```mermaid
flowchart LR
  A[右键平移太阳居中] --> B[滚轮拉近 dolly]
  B --> C{centerDepth 每帧重定目标}
  C -->|视线漂移扫过地球| D[参考深度 1AU → 0.5R 突变]
  D --> E[累计比例不变 → 单帧猛冲/倒退 = 跳回地球]
```

## 复现结论
✅ 全部缺陷已确认（6 FAIL + 1b 机制级确认），可进入修复。
