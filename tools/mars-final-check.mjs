...
async function waitMode(t,ms=15000){
  const t0=Date.now();
  while(Date.now()-t0<ms){try{if((await page.evaluate(()=>window.__game?.getMode?.()))===t)return true;}catch{}await sleep(250);}
}
...
await page.evaluate(async ()=>{
  ...
  function elev(){
    ...
    return Math.asin(Math.max(-1,Math.min(1,s[0]*ux+s[1]*uy+s[2]*uz)))*180/Math.PI;
  }
  for(let i=0;i<40;i++){
  ...
  w.yaw=Math.atan2(hx/hl,hz/hl)+Math.PI; // 背太阳
  w.pitch=1.0; // 约 57° 仰望天空
  w.smx=w.yaw; w.smy=w.pitch;
});
await sleep(4000);
await page.screenshot({ path: OUT+'mars-day-zenith.png' });
console.log('✅ 白昼天顶截图');
...
await page.evaluate(async ()=>{
  ...
  function sunElevAndDir(){
    ...
    const hx=sun3[0]-dot*ux,hy=sun3[1]-dot*uy,hz=sun3[2]-dot*uz;
    const hl=Math.sqrt(hx*hx+hy*hy+hz*hz)||1;
    return { elev:Math.asin(Math.max(-1,Math.min(1,dot)))*180/Math.PI, hx:hx/hl, hz:hz/hl };
  }
  for(let i=0;i<40;i++){
    await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));
    const {elev}=sunElevAndDir();
    if(elev>=4&&elev<=14) break;
    clock.jdTT+=STEP;
    await new Promise(r=>requestAnimationFrame(r));
  }
  const {elev,hx,hz}=sunElevAndDir();
  // 朝向太阳，仰角约 3° 看地平线
  w.yaw=Math.atan2(hx,hz);
  w.pitch=0.05;
  w.smx=w.yaw; w.smy=w.pitch;
  return elev;
});
await sleep(5000);
await page.screenshot({ path: OUT+'mars-sunset-final.png' });
console.log('✅ 日落截图');

// 仰头 45° 看太阳周围天空
await page.evaluate(()=>{ const w=window.__game.ship.walk; w.pitch=0.8; w.smy=0.8; });
await sleep(2000);
await page.screenshot({ path: OUT+'mars-sunset-corona.png' });
console.log('✅ 日落光晕截图');
